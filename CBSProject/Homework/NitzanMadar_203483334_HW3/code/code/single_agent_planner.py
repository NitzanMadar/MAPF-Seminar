...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if next_time not in constraint_table:
        if INF in constraint_table:
            # is there inf constraint in the next location at timestep smaller then next timestep:
            return len(list(filter(lambda inf_con:
                                   inf_con[0] <= next_time and next_loc in inf_con[1], constraint_table[INF]))) > 0
        else:
            return False
    # edge constraints:
    if INF in constraint_table:
        return ([next_loc] in constraint_table[next_time]) or ([curr_loc, next_loc] in constraint_table[next_time]) \
                or ((next_time, next_loc) in constraint_table[INF])
    else:
        return ([next_loc] in constraint_table[next_time]) or ([curr_loc, next_loc] in constraint_table[next_time])
    #
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    while len(open_list) > 0:
        ...
        for dir in range(5):  #  changed to 5 because I added dir=5 stay, see "move" function
            ...
            if child['timestep'] > 4 * len(my_map) * len(my_map[1]):
                continue
