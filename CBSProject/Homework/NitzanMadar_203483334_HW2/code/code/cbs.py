...
def detect_collisions(paths):
    ...
    for i in range(len(paths)):
        for j in range(i+1, len(paths)):
            loc_t = detect_collision(paths[i],paths[j])
            if loc_t!=None:
                if len(loc_t[0])==1:
                    collisions.append({'a1':i,'a2':j,'loc':loc_t[0],'timestep':loc_t[1]})
                if len(loc_t[0]) == 2:
                    collisions.append({'a1': i, 'a2': j, 'loc': loc_t[0], 'timestep': loc_t[1]+1})
    return collisions
